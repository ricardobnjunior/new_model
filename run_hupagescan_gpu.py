...
path_to_save_new_model = r'E:\Doutorado - ultima fase\code\HU-PageScan\checkpoint'
output_refined = r'E:\Doutorado - ultima fase\code\HU-PageScan\output_refined'

if not os.path.exists(output_refined):
    os.makedirs(output_refined)

if not os.path.exists(path_to_save_new_model):
    os.makedirs(path_to_save_new_model)
...
def get_model():
    ...
    model.compile(optimizer=adam_v2.Adam(lr=float(arguments['--lr'])), loss=dice_coef_loss, metrics=[dice_coef])
    return model
